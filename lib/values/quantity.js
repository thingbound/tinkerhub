...
class Factory {
    constructor(name, base, conversions) {
        this.name = name;
        this.base = base;
        ...
        parsing.value = new RegExp('\\s*(' + NUMBER + ')\\s*(.+)?\\s*');
    ...
    _parse(value) {
        const parts = this.parsing.value.exec(value);
        if(! parts) {
        ...
        return this._instance(parseFloat(number), unit);
    }
}
...
class QuantityBuilder {
    ...
    build() {
        const factory = new Factory(this.name, this.base, this.conversions);
