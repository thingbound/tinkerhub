...
module.exports = class Instance {
	...
	children() {
		if(! this.matches('cap:children')) {
			return this.call('children', arguments);
		}

		return this.layer.get('parent:' + this.id);
	}

	child(id) {
		if(! this.matches('cap:children')) {
			return this.call('child', arguments);
		}

		let target;
		if(id.indexOf(':') > 0) {
			target = matchers.or(id, this.id + ':' + id);
		} else {
			target = this.id + ':' + id;
		}

		const parentTag = 'parent:' + this.id;
		const childCollection = this.layer.get(target);

		for(const c of childCollection) {
			if(c.matches(parentTag)) {
				return c;
			}
		}
