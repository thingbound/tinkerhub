...
module.exports = class Services {
	...
	_handleNodeAvailable(node) {
		/*
		 * When we connect to a new node, send the ndoe all of the services
		 * we know about.
		 */
	...
	_handleMessage(msg) {
		switch(msg.type) {
			...
			case 'service:invoke':
				...
				break;
			case 'service:invoke-progress':
				this._handleServiceInvokeProgress(msg.returnPath, msg.payload);
				break;
	...
	_handleServiceSubscribe(node, data) {

	}

	_handleServiceUnsubscribe(node, data) {
	...
	_handleServiceInvoke(node, message) {
		...
		if(! (service instanceof LocalService)) {
			...
			// TODO: Readd progress support
		}
	}

	_handleServiceInvokeProgress(node, data) {
